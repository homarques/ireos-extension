...
public class SVMExamples implements Serializable {
	...
	/**
	 * The labels of the examples if known. -1 and +1 for classification or the real
	 * value for regression tasks. Will be filled by prediction.
	 */
	private double[] ys;
	...
	public SVMExamples(SVMExamples a, int knn[], int outlierIndex) {
		this(knn.length + 1, 0.0d);
		dim = a.get_dim();
		for (int i = 0; i < knn.length; i++) {
			ids[i] = a.ids[knn[i]];
			ys[i] = -1;
			cs[i] = a.cs[knn[i]];
			atts[i] = new double[a.atts[knn[i]].length];
			index[i] = new int[a.index[knn[i]].length];

			for (int j = 0; j < a.atts[knn[i]].length; j++) {
				atts[i][j] = a.atts[knn[i]][j];
				index[i][j] = a.index[knn[i]][j];
			}
		}
		
		ids[knn.length] = a.ids[outlierIndex];
		ys[knn.length] = 1;
		cs[knn.length] = this.getC();
		atts[knn.length] = new double[a.atts[outlierIndex].length];
		index[knn.length] = new int[a.index[outlierIndex].length];

		for (int j = 0; j < a.atts[outlierIndex].length; j++) {
			atts[knn.length][j] = a.atts[outlierIndex][j];
			index[knn.length][j] = a.index[outlierIndex][j];
		}
	}
	 ...
	 * @throws IOException
	 */
	public SVMExamples(BufferedReader reader, int size, double cs[]) throws IOException {
		this(size, 0.0d);
	 ...
	 * @throws IOException
	 */
	public SVMExamples(BufferedReader reader, int size, double c) throws IOException {
		this(size, 0.0d);
	...
	/**
	 * Gets an alpha-value. Please note that the alpha values are already multiplied
	 * by the corresponding y-value.
	 * 
	 * @param pos
	 *            Number of example
	 * @return alpha
