...
public class MaximumMarginClassifierThread extends Thread {
	...
	private HashMap<Double, Pair> gammaSeparability = new HashMap<>();
	...
	public MaximumMarginClassifierThread(SVMExamples model, double[] gammas, int outlierIndex, int[] knn) {
		if (model != null) {
			...
			this.model = null;
		/*
		 * if (outlierIndex > -1) { this.model.set_y(outlierIndex, 1);
		 * this.model.set_cs(outlierIndex, model.getC()); // System.out.println("."+
		 * outlierIndex); }
		 */
		this.gammas = gammas;
